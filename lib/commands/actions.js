...
const CONTAINER_PATH_PATTERN = new RegExp(
  `^${CONTAINER_PATH_MARKER}([^/]+)/(.+)`
);
const ANDROID_MEDIA_RESCAN_INTENT =
...
commands.pinchClose = async function(
  startX,
  startY,
  endX,
  endY,
...
commands.pinchOpen = async function(
  startX,
  startY,
  endX,
  endY,
...
commands.pullFile = async function(remotePath) {
  ...
  if (remotePath.startsWith(CONTAINER_PATH_MARKER)) {
    ...
    try {
      await this.adb.shell([
        'run-as',
        packageId,
        `chmod 777 '${pathInContainer.replace(/'/g, "\\'")}'`,
      ]);
  ...
  try {
    await this.adb.pull(
      _.isString(tmpDestination) ? tmpDestination : remotePath,
      localFile
    );
...
commands.pushFile = async function(remotePath, base64Data) {
  ...
  try {
    ...
    if (remotePath.startsWith(CONTAINER_PATH_MARKER)) {
      const [packageId, pathInContainer] = parseContainerPath(remotePath);
      log.info(
        `Parsed package identifier '${packageId}' from '${remotePath}'. Will put the data into '${pathInContainer}'`
      );
      tmpDestination = `/data/local/tmp/${path.posix.basename(
        pathInContainer
      )}`;
      try {
        await this.adb.shell([
          ...
          packageId,
          `mkdir -p '${path.posix
            .dirname(pathInContainer)
            .replace(/'/g, "\\'")}'`,
        ]);
        await this.adb.shell([
          'run-as',
          packageId,
          `touch '${pathInContainer.replace(/'/g, "\\'")}'`,
        ]);
        await this.adb.shell([
          'run-as',
          packageId,
      ...
      try {
        ...
        log.warn(
          `Got error broadcasting media scan intent: ${e.message}; ignoring`
        );
      }
    }
  } finally {
...
helpers.getScreenshotDataWithAdbExecOut = async function(adb) {
  let {stdout, stderr, code} = await exec(
    adb.executable.path,
    adb.executable.defaultArgs.concat([
      'exec-out',
      '/system/bin/screencap',
      '-p',
    ]),
  ...
  if (code || stderr.length) {
    throw new Error(
      `Screenshot returned error, code: '${code}', stderr: '${stderr.toString()}'`
    );
  }
